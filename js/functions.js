...
function showOutsider() {
    ...
    outsider[1].Multiplier = linearX(outsider[1].Level, 10);
    ...
    //show data
    tb.rows[1].cells[2].innerHTML = "+" + sciFormat(outsider[1].Multiplier.times(100), 4) + "% of Revolc per level";
...
function compute(x) {
    ...
    switch ($("#otherAncientsLowerBase").val()) {
        case "11":
            ...
            break;
        case "12":
            var base_level_for_no_formula = getFortunaLevel(m, true);
            break;
        case "13":
            var base_level_for_no_formula = getAtmanLevel(m, alpha, true);
            break;
        case "14":
            var base_level_for_no_formula = getDoraLevel(m, true);
            break;
        case "17":
            var base_level_for_no_formula = getChronosLevel(m, true);
            break;
        case "18":
            var base_level_for_no_formula = getBubosLevel(m, true);
            break;
        case "21":
            var base_level_for_no_formula = getKumaLevel(m, alpha, true);
            break;
    ...
    for (var i in ancient) {
        if (ancient[i].Level.gt(0)) {
            switch (i) {
                ...
                case "33": //Knobur
                case "31": //Revolc
                ancient[i].OptimalLevel = Decimal.min(7486, Decimal.max(base_level_for_no_formula.minus($("#otherAncientsLower").val(), 0)));
                    break;
                case "22": //Chawedo
